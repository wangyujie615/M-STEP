...
def generate_heatmap_floor(bboxes, patch_size=320, stride=16):
    """
    Generate ground truth heatmap same as CenterNet
    Args:
        bboxes (torch.Tensor): shape of [num_search, bs, 4]
        patch_size (int or tuple of ints): int for square patch, tuple for (h,w) rectangle patch
    Returns:
        gaussian_maps: list of generated heatmap
    ...
    heatmap_size = torch.div(patch_size, stride, rounding_mode='floor')
    for single_patch_bboxes in bboxes:
        bs = single_patch_bboxes.shape[0]
        gt_scoremap = torch.zeros(bs, *heatmap_size)
        classes = torch.arange(bs).to(torch.long)
        batched_heatmap_size = torch.tensor([heatmap_size[1],heatmap_size[0]]).repeat(2)[None,:]
        bbox = single_patch_bboxes * batched_heatmap_size.to(single_patch_bboxes.device)
        wh = bbox[:, 2:]
        centers_int = (bbox[:, :2] + wh / 2).floor()
        CenterNetHeatMap.generate_score_map(gt_scoremap, classes, wh, centers_int, 0.7)
        gaussian_maps.append(gt_scoremap.to(bbox.device))
    return gaussian_maps
