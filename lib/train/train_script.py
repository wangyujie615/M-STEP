...
def run(settings):
    ...
    # Create network
    if settings.script_name == "m_step":
        from lib.models.m_step import build_m_step
        from lib.train.actors import M_STEPActor
        net = build_m_step(cfg)
    else:
    ...
    if settings.script_name == "m_step":
        ...
        actor = M_STEPActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
