...
from lib.models.m_step import build_m_step
from lib.test.tracker.basetracker import BaseTracker
import torch
from pytorch_grad_cam import EigenCAM
from pytorch_grad_cam.utils.image import show_cam_on_image, scale_cam_image
from lib.test.tracker.vis_utils import gen_visualization,vis_attn_maps
...
from lib.test.tracker.data_utils import Preprocessor,Preprocessor_change
from lib.utils.box_ops import clip_box
from lib.utils.ce_utils import generate_mask_cond

class M_STEP(BaseTracker):
    def __init__(self, params):
        super(M_STEP, self).__init__(params)
        network = build_m_step(params.cfg, training=False)
        network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)
        self.cfg = params.cfg
        self.network = network.cuda()
        self.network.eval()
        self.preprocessor = Preprocessor()
        self.preprocessor_change = Preprocessor_change()
        self.state = None
    ...
    def initialize(self, image, info: dict):
        ...
        with torch.no_grad():
            ...
            self.memory_frames = [template.tensors] #最开始记忆的是模板帧
    ...
    def track(self, image, info: dict = None):
        ...
        search_vis = self.preprocessor_change.process(x_patch_arr, x_amask_arr)
        ...
        self.memory_frames.append(frame) # 记忆存储
        if self.cfg.MODEL.BACKBONE.CE_LOC:  # use CE module
